...
import { generateRandomLeidenLocation, calculateDistance, calculateScore } from './utils/locationUtils';
...
const App = () => {
    ...
    const [actualStreetViewLocation, setActualStreetViewLocation] = useState(null);
    ...
    const startNewRound = () => {
        ...
        // Set a timeout to detect if Street View fails to load
        setTimeout(() => {
            if (!streetViewReady && !actualStreetViewLocation) {
                console.warn('⚠️ Street View failed to load after 5 seconds, trying new location...');
                setStreetViewError(true);
                // Try a new location after a short delay
                setTimeout(() => {
                    const newLocation = selectRandomLocation();
                    setCurrentLocation(newLocation);
    ...
    const handleGuess = () => {
        ...
        if (!actualStreetViewLocation) {
            console.warn('⚠️ No actual Street View location available, using generated location as fallback');
        }
    ...
    const handleNameSubmit = async (playerName) => {
        try {
            const result = await highScoresService.addHighScore(playerName, score, totalDistance, 3);
            if (result.success) {
                ...
                <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full mx-4 text-center">
                    ...
                    <div className="space-y-3">
                        ...
                        <button
                            ...
                            className="w-full  text-white py-2 px-4 rounded-md hover:bg-gray-700 transition-colors"
                        >
                            Terug naar Setup
                        </button>
                    </div>
        ...
        <div className="min-h-screen ">
            ...
            {gameState !== 'setup' && (
                <div className="flex h-screen">
                    <div className="flex-1 p-2 pace-y-4 ">
